---
title: R-Exams - Fexible exam result reports for NOPS exams
author: kenjisato
date: '2021-02-17'
slug: r-exams-fexible-exam-result-reports-for-nops-exams
categories: []
tags: ["R", "exams"]
draft: true
---
...
In [Revision 1730](https://r-forge.r-project.org/scm/viewvc.php/pkg/exams/R/nops_eval.R?view=markup&revision=1730&root=exams), however, Achim Zeileis made a very nice update to the package: We can now override `nops_eval_write()` with `flavor` option. With
...
```{r, eval=FALSE}
nops_eval_eval()
nops_eval_write_mywriter()
```

So, if you want to customize result reports, define
...
`nops_eval_write_template()` use templates (HTML or anything) that are separated from R code. 
...
Behind the scene, `nops_eval_write_template()` makes use of [**whisker** package](https://cran.r-project.org/web/packages/whisker/index.html). 

In the above template, you see such placeholders as `{{language}}`, `{{answer}}`. To write your own template you may want to know what placeholders you can use. 
...
To access `dat$points`, you use `{{points}}`, for `data$DCF$RegistrationNumber`, `dat$DCF$GivenName`, 
...
**NB**: You use `{{#outer}}{{inner}}{{/outer}}` for three purposes: (1) element access, (2) for-each pattern and (3) if clause.
...
# 2
for (element in dat$outer$inner) element$something

# 3
if (dat$outer) dat$inner
```
...
Since `nops_eval_write_template()` does not assume that the template file is in HTML, you can use whatever plain text format you like. For instance, you can write a LaTeX template, say, "custom.tex" and run 
...
If the language file (dcf file) of you choice has LaTeX commands and if you want to keep them in the resulting exam reports (in tex format), then you can set converter = "none". 
...
This should be necessary if your language use characters outside of OT1 and the dcf file of your choice does not properly set LaTeX preamble to handle those unicode characters. 

### Installed templates in txt and tex formats

You can use "file" parameter when you are fine with the installed templates.
...
`nops_eval_write_template()` searches for installed templates based on the extension of `file` argument. Currently, "tex", "txt", and "html" (default) are available.

### Post-processing

Now that you know how to create LaTeX source files. You'd like to compile them into PDF files, right?
You can do so with a function passed to the "post_process" argument.
...
If you delete the source file, you can write a post-processor as below:
...
If you are fine with returning two files: (1) result report in any format and (2) scanned image in PNG format, then you can simply use "return_scan = TRUE", i.e.,
...
Maybe you want to use a LaTeX template and compile the results into PDF, each of which include scanned PNG files. 

You may want to use a template like  below (say, "template.tex"):
...
Call `nops_eval()` with "return_scan = TRUE".
